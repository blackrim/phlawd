...
"""
very simple script to put new names from names.dmp into a sqlite database
...
if __name__ == "__main__":
	if len(sys.argv) != 4:
		print("usage: update_names_dmp_sqlite database namesfile nodesfile")
		sys.exit(0)
	...
	for i in filen:
		...
		#print(cmdin)
		try:
			#test for whether it is already in there
			cur.execute("select ncbi_id from taxonomy where ncbi_id = ?;",(gin,))
			a = cur.fetchall()
			if len(a) == 0:
				print(gin,nm,ednm)
